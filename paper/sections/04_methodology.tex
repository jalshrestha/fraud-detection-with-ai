...
Figure~\ref{fig:featdist} confirms that several features are
statistically distinguishable between fraud and benign contracts in
our labelled subset.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/feature_dist.pdf}
    \caption{Box-plot distributions of all eight node features for
    labelled fraud (red) and benign (blue) contracts (304 samples).
    In-degree, mean in-value, and unique in-counterparties show the
    largest separation, validating the importance of value-flow and
    counterparty features beyond simple degree counts.}
    \label{fig:featdist}
\end{figure}
...
schedule~\cite{loshchilov2017cosine} is configured over a maximum
100-epoch budget, decaying the learning rate from $10^{-3}$ to
$10^{-5}$.  In practice, early stopping terminates training well before
epoch 100.  Gradient clipping at norm 1.0 suppresses the early-epoch
instability that occurs before the class-weighted loss stabilises.
...
reported.  Our fusion model achieved its best validation F1 of
\textbf{0.966} at epoch 21 and early-stopped at epoch 41.
