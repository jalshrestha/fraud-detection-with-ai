...
The fusion model trains under AdamW with $\mathrm{lr}=10^{-3}$,
cosine-annealing schedule, and early stopping on validation fraud F1.
Training loss decreases from 6.66 in epoch 1 to below 0.50 by epoch 25,
and no NaN losses are observed at any point.  The model early-stops at
epoch 41 (best checkpoint at epoch 21) with the best validation fraud
F1 of \textbf{0.966}, and that checkpoint is reloaded for test-set
evaluation.
Each epoch takes approximately 34 seconds on a CPU; GPU acceleration
would reduce this to under five seconds per epoch.
...
47 test samples.  The AUC-ROC of 0.961 and AUC-PR of 0.959 confirm
strong ranking performance well above the random baseline, while the
MCC of 0.861 indicates robust detection independent of class balance.
...
Table~\ref{tab:ablation} compares six models trained on the identical
70/15/15 split under identical hyperparameters.
